export class ApiError extends Error {
    ...
    constructor(status: number, message: string) {
        super(message);
        this.status = status;
        this.name = 'ApiError'; // Set the name property for better error identification
    }
}
...
export const handleApiError = (error: unknown): { status: number; message: string; location?: string } => {
    if (error instanceof ApiError) {
        // Custom application error
        return { status: error.status, message: error.message };
    } else if (error instanceof RedirectError) {
        // Redirect error
        return { status: error.status, message: error.message, location: error.location };
    } else if (error instanceof Error) {
        // Check for specific error messages
        if (error.message.includes('Json deserialize error')) {
            return { status: 400, message: 'Invalid data format received from the server.' };
